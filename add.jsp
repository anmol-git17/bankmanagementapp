...
%>
	...
	<style> 
		...
		*{font-family: 'Archivo Black', sans-serif;text-decoration:none;margin:0px;padding:0px;}
		.button{background-color:#31c766;color:white;height:60px ; width:200px;font-size:15px;} 
		.upper{background-color:#39434d;color:white;padding-top:10px;padding-bottom:10px;margin-bottom:20px;}
...
<center>
	...
	<h1> Add User</h1>
	<br>
	<h2>Enter the details of the user you want to add.</h2>
	<br>
	</div>
	<form method="POST">
		<input class=tb type=number name=userid placeholder="create userID" required>
		<input class=tb type=number name=useridconf placeholder="confirm userID" required><br><br>
		<input class=tb type=text name=username placeholder="enter username" required>
		<input class=tb type=text name=usernameconf placeholder="confirm username" required><br><br>
		<input class=tb type=number name=bal placeholder="enter initial account balance " required>
		<input class=tb type=number name=balconf placeholder="confrim initial account balance " required><br><br>
		<input class=tb type=password name=pw placeholder="create password " required>
		<input class=tb type=password name=pwconf placeholder="confirm password " required><br><br>
		<input class=button type=submit name="adduser" value="ADD USER"><br>
	</form>
	<br>
...
try{
	...
	if(f3==2)
	{
		...
		{
			out.println("User with UserID : "+uid1+" already exists" );
	...
	if(f4==0)
	{
		...
		{
			out.println("Password Length should be between 8-15 characters!");
		}
		else
		{
			f4=2;
